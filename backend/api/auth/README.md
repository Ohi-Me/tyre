...
Shared backend modules for the `/auth` domain.
...
Next.js route handlers in `frontend/web/app/api/v1/auth/` import from here:

```ts
import { listLoads, createLoadSchema } from "@tyre/api/auth";
```
